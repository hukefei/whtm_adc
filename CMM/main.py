...
def process(path):
    ...
    return {'model': my_model, 'classes': classes}
...
def predict(img, *args, **kwargs):
    ...
    img_ = transformer(img_).unsqueeze(0)
    img_ = img_.cuda()
    result = model(img_)
...
def resnet18_cbam(checkpoint, **kwargs):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=23):
        ...
        # self.layer01 = self._make_layer(block, 64, 1, stride=2)
        # self.layer02 = self._make_layer(block, 64, 1, stride=2)
